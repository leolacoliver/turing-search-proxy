...
- YOE minimum is a hard constraint: if query says "5+ years of Python", candidate MUST show 5+ years of Python specifically, not just overall experience
- Do NOT count total career years as a substitute for years in the specific required technology
...
- If query specifies a language/framework (e.g. "Python backend"), it becomes a hard constraint
- If query specifies YOE (e.g. "5+ years"), candidate MUST meet it — do NOT substitute total career years
- LLM/API/JSON/function-calling experience = strong positive signal
- Adjacent backgrounds qualify ONLY when query is genuinely open-ended about the tech stack
- When query lists specific technical requirements (e.g. "Python + AI agents + LLM integration"), ALL must be evidenced
